...
input_emotion_scanner = EmotionDetection(
    threshold=0.5,
    blocked_emotions=["anger", "annoyance", "disappointment", "disapproval", "disgust", 
...
def test_output_scanning():
    ...
    for i, output in enumerate(test_outputs, 1):
        ...
        try:
            sanitized_output, is_valid, risk_score = scan_output([output_emotion_scanner], "", output)
            if is_valid:
                print(f"✅ Valid (Risk: {risk_score:.2f})")
            else:
                print(f"❌ Blocked (Risk: {risk_score:.2f})")
...
def test_full_emotion_analysis():
    ...
    full_output_scanner = EmotionDetection(
        threshold=0.5,
    ...
    for i, prompt in enumerate(test_prompts, 1):
        ...
        try:
            ...
            sanitized_prompt, is_valid, risk_score, full_analysis = full_output_scanner.scan_with_full_output(prompt)
...
def test_output_full_emotion_analysis():
    ...
    # Create scanner with full output mode
    full_output_scanner = OutputEmotionDetection(
        threshold=0.5,
        return_full_output=True
    )
    ...
    for i, output in enumerate(test_outputs, 1):
        ...
        try:
            ...
            # Scan with full output
            sanitized_output, is_valid, risk_score, full_analysis = full_output_scanner.scan_with_full_output("", output)
            print(f"Valid: {is_valid}, Risk: {risk_score:.2f}")
            print(f"Full Analysis: {full_analysis}")
