...
class LLMGuardPromptChain(Chain):
    ...
    output_key: str = "sanitized_prompt"  #: :meta private:
    ...
    def _call(
        ...
        """
        Executes the scanning process on the prompt and returns the sanitized output.

        This internal method performs the scanning process on the prompt. It uses the
        provided scanners to scan the prompt and then returns the sanitized output.
...
prompt = ChatPromptTemplate.from_messages(
    [
        ...
        HumanMessagePromptTemplate.from_template("{sanitized_prompt}"),
    ]
)
...
llm_guard_prompt_scanner = LLMGuardPromptChain(
    vault=vault,
    scanners={
        "Anonymize": {"use_faker": True, "use_onnx": True},
        "BanSubstrings": {
            "substrings": ["Laiyer"],
            "match_type": "word",
        ...
        "BanTopics": {"topics": ["violence"], "threshold": 0.7, "use_onnx": True},
        "Code": {"denied": ["go"], "use_onnx": True},
        "Language": {"valid_languages": ["en"], "use_onnx": True},
        "PromptInjection": {"threshold": 0.95, "use_onnx": True},
        "Regex": {"bad_patterns": ["Bearer [A-Za-z0-9-._~+/]+"]},
        "Secrets": {"redact_mode": "all"},
        "Sentiment": {"threshold": 0.0},
        "TokenLimit": {"limit": 4096},
        "Toxicity": {"threshold": 0.8, "use_onnx": True},
    },
...
guarded_chain = llm_guard_prompt_scanner | chain
